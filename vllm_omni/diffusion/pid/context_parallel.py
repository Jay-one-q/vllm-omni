...
# Original: NVIDIA CORPORATION & AFFILIATES — pid/_src/utils/context_parallel.py
#
...
def cat_outputs_cp_with_grad(x: Tensor, seq_dim: int, cp_group: ProcessGroup) -> Tensor:
    ...
    It retains computational graph locally for each rank by replacing the portion
    of the tensor with original output.
...
def robust_broadcast(tensor: torch.Tensor, src: int, pg: ProcessGroup, is_check_shape: bool = False) -> torch.Tensor:
    ...
    Args:
        ...
        src (int): The source rank for the broadcast.
