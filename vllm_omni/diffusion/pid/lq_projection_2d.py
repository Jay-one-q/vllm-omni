...
#
# ControlNet-style injection gates:
#   "sigma_aware_per_token":         per-token scalar gate (B,N,1; v1pt5)
#   "sigma_aware_per_token_per_dim": per-token per-dim gate (B,N,D; v1)
...
class LQProjection2D(nn.Module):
    ...
    Args:
        ...
        lq_aux_rgb_head_latent_block_idx: 1-based ResBlock index inside latent_proj
            where the aux RGB head should tap features. -1 uses the final merged
    ...
    def __init__(
        ...
        # --- Aux RGB head (training-only; module exists for checkpoint compat) ---
        if lq_aux_rgb_head:
    ...
    def forward(
        ...
        Returns:
            ...
            If return_aux=True, returns (outputs, aux_dict).
        """
